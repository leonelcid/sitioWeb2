<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link rel="stylesheet" href="estilos.css">
</head>
<body>
<div class="container">
	<header>
		<h1>Programaci&oacuten en PHP y MySQL - Nivel Avanzado</h1>
	<nav>
		<?php include("botonera.php"); ?>
	<!-- </nav> -->
	</header>
	<section>
		<h2>Mi carrito</h2>
		<nav>
		<?php
			include('includes.php');

			$cod = $_POST['codigo'];
			$pro = $_POST['producto'];
			$desc = $_POST['descripcion'];
			$pre = $_POST['precio'];

			// Añade a la tabla compras
			$anadir = $carrito->insertarProducto($cod, $pro, $desc, $pre);

			// Redireccionamos a unidad7
			header("Location: unidad7.php");
			// Muestra lo que llega por POST, es solo para verificar que el dato llega
			//echo $_POST['codigo'];
		?>
		</nav>
	</section>
	<aside>

  	</aside>
	<footer>
		<a href="https://site.elearning-total.com/courses/?com=lb">Programaci&oacuten en PHP y MySQL - Nivel Avanzado</a>
	</footer>
 
</div>
</body>
</html>